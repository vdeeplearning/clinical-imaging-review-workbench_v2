    ...
    with get_connection() as conn:
        rows = conn.execute(
            ...
            SELECT
                ...
                s.created_at,
                COUNT(l.id) AS lesion_count,
                COALESCE(SUM(l.long_diameter), 0) AS total_burden
            FROM scans s
...
def save_scan(patient_fk: int, scan_date: str, accession_number: str, lesions: list[Lesion]) -> int:
    """
    Save a new scan and all associated lesions. Returns the new scan ID.
    """
    ...
    with get_connection() as conn:
        cursor = conn.execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (patient_fk, scan_date, accession_number.strip() or None, created_at),
        )
...
def delete_scan(scan_id: int) -> bool:
    """
    Delete a scan and all associated lesions.
...
def delete_patient(patient_fk: int) -> bool:
    """
    Delete a patient and all associated scans + lesions.
...
def get_scan_detail(scan_id: int) -> Optional[ScanDetail]:
    """
    Return a full scan detail with all lesions, or None if not found.
    """
    with get_connection() as conn:
